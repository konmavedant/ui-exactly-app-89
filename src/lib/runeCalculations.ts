...
export async function calculateRuneTime(location: string): Promise<RuneTimeInfluence> {
  try {
    ...
    // Big arm (hour hand) always points to Aries at 60°
    const hourRotation = 60;

    // Small arm (minute hand) moves based on 24-hour clock (15 degrees per hour)
    // 360° / 24 hours = 15° per hour
    // Add minute contribution: (minutes / 60) * 15 for smooth movement
    const minuteRotation = (hours * 15) + ((minutes / 60) * 15);
